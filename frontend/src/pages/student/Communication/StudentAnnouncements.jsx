  ...
  const markRead = async (id, isRead) => {
    ...
    try {
      ...
      setAnnouncements((prev) =>
        prev.map((ann) =>
          ann._id === id
            ? { ...ann, readBy: [...(ann.readBy || []), { user: studentId }] }
            : ann,
        ),
      );
    } catch (err) {
      ...
      {announcements.length === 0 ? (
        <div className="bg-white border border-dashed border-slate-300 rounded-3xl p-20 text-center">
          ...
          <p className="text-slate-500">
            There are no active announcements on your board right now.
          </p>
        </div>
      ) : (
            ...
            return (
              ...
              >
                ...
                >
                  {/* Status Icon */}
                  <div
                    className={`mt-1 flex-shrink-0 w-10 h-10 rounded-2xl flex items-center justify-center transition-colors ${
                      isRead
                  ...
                  <div className="px-5 md:px-6 pb-6 pt-2 border-t border-slate-50 animate-in slide-in-from-top-2 duration-300">
                    ...
                    {ann.attachments?.length > 0 && (
                      <div className="mt-6 flex flex-col gap-2">
                        ...
                        <div className="mt-3 pt-6 border-t border-slate-100 animate-in fade-in zoom-in-95">
                          ...
                          <div className="flex justify-between items-center text-[10px] font-bold text-slate-300 uppercase tracking-[0.2em]">
                            ...
                            <span>
                              Post by: {ann.createdByName || "System"}
                            </span>
                          </div>
                        </div>
                      </div>
