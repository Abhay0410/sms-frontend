...
export default function LeaveRequests() {
  ...
  const handleAction = async (action) => {
    try {
      ...
      const endpoint = API_ENDPOINTS.ADMIN.HR.PROCESS_LEAVE(
        confirmationData.id,
      );
      ...
      await api.put(endpoint, {
        ...
        adminRemarks:
          remarks ||
          `Leave ${action === "APPROVED" ? "approved" : "rejected"} by admin`,
      });
                ...
                {requests.map((req) => {
                  const displayPhoto = req.teacherId?.profilePicture
                    ? req.teacherId.profilePicture.startsWith("http")
                      ...
                      : `${API_URL}/uploads/${req.teacherId.schoolId}/teachers/${req.teacherId.profilePicture}`
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="p-4">
                        <div>
                          <span
                            className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-sm font-medium ${getLeaveTypeColor(req.leaveType)}`}
                          >
                            {leaveTypeOptions.find(
                              (opt) => opt.value === req.leaveType,
                            )?.icon &&
                              React.createElement(
                                leaveTypeOptions.find(
                                  (opt) => opt.value === req.leaveType,
              ...
              <div className="space-y-6">
                ...
                <div className="flex items-center gap-4 p-4 bg-slate-50 rounded-xl">
                  ...
                  <div>
                    <h4 className="text-lg font-bold text-slate-900">
                      {selectedRequest.teacherId?.name}
                    </h4>
                    <p className="text-slate-600">
                      {selectedRequest.teacherId?.teacherID} •{" "}
                      {selectedRequest.teacherId?.department ||
                        "General Department"}
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="p-4 bg-slate-50 rounded-xl">
                    ...
                    <p
                      className={`mt-2 px-3 py-1.5 rounded-lg inline-flex items-center gap-2 ${getLeaveTypeColor(selectedRequest.leaveType)}`}
                    >
                      {leaveTypeOptions.find(
                ...
                <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <span
                      className={`px-4 py-2 rounded-full text-sm font-bold ${getStatusColor(selectedRequest.status)}`}
                    >
