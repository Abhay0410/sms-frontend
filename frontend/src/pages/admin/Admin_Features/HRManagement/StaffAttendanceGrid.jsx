...
export default function StaffAttendanceGrid() {
  ...
  const fetchMatrix = useCallback(async () => {
    ...
    try {
      const res = await api.get(
        `/api/admin/payroll/matrix?month=${month}&year=${year}&academicYear=${academicYear}`,
      );
  ...
  const fetchSessions = async () => {
    try {
      ...
      // }
      if (active && !localStorage.getItem("academicYear")) {
  ...
  const filteredData = useMemo(() => {
    return data.matrix.filter((employee) => {
      ...
      const matchesStatus =
        statusFilter === "ALL" ||
        (employee.attendance &&
          Object.values(employee.attendance).includes(statusFilter));
                ...
                <div className="flex items-center gap-3">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      ...
                      // ✅ IMPORTANT: year bhi update karo
                  ...
                  <button
                    onClick={fetchMatrix}
                    disabled={loading}
                    className="p-3 bg-amber-500 hover:bg-amber-600 text-white rounded-xl hover:shadow-xl transition-all disabled:opacity-50"
                  >
