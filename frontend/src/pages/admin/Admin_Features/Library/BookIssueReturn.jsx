        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white rounded-2xl shadow-sm border border-slate-400 overflow-hidden">
              <div className="flex bg-slate-50 p-2 border-b border-slate-400">
                <button
                  onClick={() => setActiveTab("ISSUE")}
                  className={`flex-1 py-4 rounded-2xl font-bold transition-all flex items-center justify-center gap-2 ${
                    activeTab === "ISSUE"
                      ? "bg-white shadow-sm text-orange-600 border border-slate-400" : "text-slate-500"
                  }`}
                >
                ...
                <button
                  ...
                  className={`flex-1 py-4 rounded-2xl font-bold transition-all flex items-center justify-center gap-2 ${
                    activeTab === "RETURN"
                      ? "bg-white shadow-sm text-emerald-600 border border-slate-400" : "text-slate-500"
                  }`}
                >
              ...
              <div className="p-10">
                {activeTab === "ISSUE" ? (
                  <form onSubmit={handleIssue} className="space-y-6">
                    <div className="flex gap-2 p-1 bg-slate-50 border border-slate-400 rounded-2xl w-fit">
                      {['student', 'teacher'].map(type => (
                        <button key={type} type="button" onClick={() => setUserType(type)} className={`px-6 py-2 rounded-2xl text-xs font-black uppercase tracking-widest transition-all ${userType === type ? "bg-white text-orange-600 shadow-sm border border-slate-400" : "text-slate-500"}`}>
                          {type}
                        </button>
                      ))}
                    </div>
                    <div>
                      ...
                      <div className="relative mt-2">
                        ...
                        <input ref={studentInputRef} required value={issueData.userId} onChange={e => setIssueData({...issueData, userId: e.target.value})} className="w-full pl-12 pr-4 py-4 bg-white border border-slate-400 rounded-2xl focus:border-orange-500 outline-none transition-all font-bold" placeholder={`Scan or enter ${userType} ID`} />
                      </div>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div>
                        <label className="text-xs font-black text-slate-500 uppercase ml-1">Due Date</label>
                        <input type="date" required value={issueData.dueDate} onChange={e => setIssueData({...issueData, dueDate: e.target.value})} className="w-full mt-2 p-4 bg-white border border-slate-400 rounded-2xl focus:border-orange-500 outline-none font-bold" />
                      </div>
                      <div>
                        <label className="text-xs font-black text-slate-500 uppercase ml-1">Book Serial</label>
                        <input required value={issueData.serialCode} onChange={e => setIssueData({...issueData, serialCode: e.target.value})} className="w-full mt-2 p-4 bg-white border border-slate-400 rounded-2xl focus:border-orange-500 outline-none font-bold" placeholder="BK-000" />
                      </div>
                    </div>
                  ...
                  <form onSubmit={handleReturn} className="space-y-6 animate-in fade-in">
                    <div>
                      ...
                      <div className="relative mt-2">
                        ...
                        <input ref={scanInputRef} required value={returnData.serialCode} onChange={e => setReturnData({serialCode: e.target.value})} className="w-full pl-12 pr-4 py-5 bg-white border border-slate-400 rounded-2xl focus:border-emerald-500 outline-none font-bold text-lg" placeholder="BK-X-000" />
                      </div>
                    </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <button 
                ...
                className="p-6 bg-white rounded-2xl border border-slate-400 shadow-sm hover:shadow-md transition-all text-left group"
              >
              ...
              <div className="p-4 space-y-3">
                {recentTransactions.length > 0 ? (
                  recentTransactions.map((t, i) => {
                    ...
                    return (
                      <div key={i} className={`p-4 rounded-2xl bg-white shadow-sm border border-slate-400`}>
      ...
      {showIssuedModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-4xl max-h-[85vh] overflow-hidden flex flex-col shadow-2xl">
            <div className="p-8 border-b border-slate-400 flex items-center justify-between">
              <div>
            ...
            <div className="flex-1 overflow-y-auto p-8">
              <table className="w-full text-left">
                <thead>
                  <tr className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] border-b border-slate-400">
