...
export default function PaymentHistory() {
    ...
    async (page = 1) => {
      try {
        ...
        const response = await api.get(
          `${API_ENDPOINTS.ADMIN.FEE.ALL}?${params}`,
        );
  ...
  const fetchFilterOptions = async () => {
    try {
      const classResponse = await api.get(
        API_ENDPOINTS.ADMIN.CLASS.LIST || "/api/admin/class/list",
      );
      const classData =
  ...
  const downloadReceipt = async (paymentId, receiptNumber) => {
    try {
      ...
      toast.error(
        `Failed to download receipt. Please check if receipt exists.`,
      );
    } finally {
        ...
        <div className="flex items-center gap-5">
          ...
          <div>
            ...
            <p className="text-gray-500 font-medium text-sm mt-1">
              View and manage all fee payment receipts
            </p>
          </div>
        </div>
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-400 overflow-hidden">
        <div className="p-6 border-b border-slate-400 flex justify-between items-center">
          ...
          <div className="text-sm text-slate-500">
            Showing{" "}
            <span className="font-bold text-indigo-600">{payments.length}</span>{" "}
            of <span className="font-bold">{totalPayments}</span> records
          </div>
        </div>
                ...
                <tbody className="divide-y divide-slate-400">
                  {payments.map((payment, index) => (
                    ...
                    >
                      ...
                      <td className="p-4">
                        <div
                          className={`${getPaymentMethodColor(payment.paymentMethod)} pl-4`}
                        >
                      ...
                      <td className="p-4">
                        <span
                          className={`px-3 py-1 rounded-full text-xs font-bold ${getStatusColor(payment.status)}`}
                        >
                          {payment.status || "N/A"}
                        </span>
                      </td>
            ...
            <div className="bg-gradient-to-r from-indigo-600 to-purple-600 p-6 text-white flex justify-between items-center">
              <div>
                ...
                <div className="flex items-center gap-4 mt-2">
                  ...
                  <span
                    className={`px-3 py-1 rounded-full text-xs font-bold ${getStatusColor(selectedReceipt.status).replace("bg-", "bg-").replace("text-", "text-")}`}
                  >
            ...
            <div className="p-8 space-y-8 max-h-[70vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50 p-6 rounded-2xl">
                ...
                <div>
                  ...
                  <p className="font-bold">
                    {selectedReceipt.student?.studentID || "N/A"}
                  </p>
                </div>
                <div>
                  <p className="text-xs text-slate-500 font-semibold uppercase">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-4">
                    <div className="p-4 bg-indigo-50 rounded-2xl">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex justify-between">
                          ...
                          <span className="font-bold">
                            {selectedReceipt._id?.slice(-8)?.toUpperCase() ||
                              "N/A"}
                          </span>
                        </div>
                        <div className="flex justify-between">
                    ...
                    <div className="p-4 bg-slate-50 rounded-2xl">
                      ...
                      <div className="flex items-center gap-3">
                        <div
                          className={`h-3 w-3 rounded-full ${selectedReceipt.status === "PAID" ? "bg-emerald-500" : selectedReceipt.status === "PARTIAL" ? "bg-amber-500" : "bg-rose-500"}`}
                        ></div>
                        <span className="font-bold">
                          {selectedReceipt.status} Payment
                        </span>
