...
export default function FeeOverview({ academicYear }) {
  ...
  try {
    ...
    const res = await api.get(
      API_ENDPOINTS.ADMIN.FEE.STUDENTS_WITH_FEES, // ✅ SAME AS WORKING CODE
      {
        params: {
          academicYear,
          status, // paid | unpaid
    ...
    <div className="space-y-8">
      ...
      <div className="rounded-2xl bg-white p-6 shadow-lg border">
        <div className="flex justify-between mb-3">
          <h3 className="text-lg font-bold">Fee Collection Progress</h3>
          <span className="text-2xl font-bold text-purple-600">
            {statistics.collectionPercentage}%
          </span>
        </div>

        <div className="w-full h-6 bg-slate-200 rounded-full">
          <div
            className="h-6 rounded-full bg-gradient-to-r from-purple-600 to-blue-600"
            style={{
              width: `${Math.min(statistics.collectionPercentage, 100)}%`,
            }}
          />
        </div>
      </div>
