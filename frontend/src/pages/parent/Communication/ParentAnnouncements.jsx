    ...
    <div className="p-6 min-h-screen bg-slate-50">
      <div className="mb-8 flex flex-col md:flex-row justify-between items-center gap-4">
        ...
        <select
          ...
          className="p-2 border rounded-lg bg-white shadow-sm outline-none ring-indigo-500 focus:ring-2"
        >
        ...
        {announcements.map((ann) => (
          ...
          >
            <div className="flex items-center gap-2 mb-3">
              <span
                className={`px-2 py-1 rounded-full text-[10px] font-bold ${ann.type === "EMERGENCY" ? "bg-red-100 text-red-600" : "bg-indigo-100 text-indigo-600"}`}
              >
                {ann.type}
              </span>
              ...
              <div className="flex flex-wrap gap-3 pt-4 border-t border-slate-50">
                {ann.attachments.map((file, i) => (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="flex items-center gap-2 text-xs font-semibold text-indigo-500 hover:underline"
                  >
