  ...
  const loadTeacherAssignments = async () => {
    try {
      ...
      if (noClasses && noSubjects) {
        toast.info("You are not assigned to any classes or subjects", { toastId: "no-assignments" });
      } else if (noClasses && !noSubjects) {
        toast.info(
          "You are a subject teacher. Only class teachers can mark attendance.",
          { toastId: "subject-teacher-warning" }
        );
      }
    } catch (error) {
      toast.error(error.message || "Failed to load your assignments", { toastId: "load-assignments-error" });
    } finally {
      setLoading(false);
    }
  };
