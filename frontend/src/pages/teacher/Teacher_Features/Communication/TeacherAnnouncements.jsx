...
export default function TeacherAnnouncements() {
  ...
  {/* <BackButton/> */}
        {isCreating ? (
          <form onSubmit={handleSubmit} className="bg-white p-6 rounded-2xl shadow-sm border border-slate-400 space-y-4 mb-6 animate-in fade-in slide-in-from-top-5">
            <input className="w-full border border-slate-400 p-3 rounded-xl outline-none focus:ring-2 focus:ring-indigo-500" placeholder="Announcement Title" required onChange={e => setFormData({...formData, title: e.target.value})} />
            <textarea className="w-full border border-slate-400 p-3 rounded-xl h-32 resize-none outline-none focus:ring-2 focus:ring-indigo-500" placeholder="Write your message..." required onChange={e => setFormData({...formData, content: e.target.value})} />
              ...
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-400">
                  ...
                  <div className="max-h-32 overflow-y-auto space-y-1 pr-2">
                      ...
                      {mySections.map((sec, idx) => (
                          <label key={idx} className="flex items-center justify-between text-xs bg-white p-2 rounded-lg border border-slate-400 cursor-pointer hover:border-indigo-300 transition-colors">
            ...
            <div className="flex items-center gap-4">
              ...
              <label className="flex items-center gap-2 cursor-pointer bg-slate-50 px-4 py-2 rounded-lg border border-slate-400 hover:bg-slate-100 transition-colors">
            ...
            {previewFiles.length > 0 && (
              <div className="flex flex-wrap gap-2 p-3 bg-slate-50 rounded-xl border border-slate-400">
                {previewFiles.map((f, i) => (
                  <div key={i} className="bg-white px-3 py-1.5 rounded-lg border border-slate-400 flex items-center gap-2 shadow-sm">
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-slate-400">
          ...
          <div className="space-y-4">
            {loading ? (
              ...
              <div key={ann._id} className={`bg-white p-6 rounded-2xl shadow-sm border border-slate-400 hover:border-indigo-200 transition-all group ${ann.isPinned ? 'bg-gradient-to-r from-orange-50/50 to-yellow-50/50 border-orange-100' : ''}`}>
                  ...
                  <div className="mt-6 pt-6 border-t border-slate-400 animate-in fade-in zoom-in-95">
                      {ann.attachments?.length > 0 && (
                          ...
                          {ann.attachments.map((file, i) => (
                              <a key={i} href={`${BACKEND_URL}${file.fileUrl}`} target="_blank" rel="noreferrer" className="flex items-center gap-3 px-4 py-2 bg-slate-50 border border-slate-400 rounded-xl text-xs text-slate-600 font-bold hover:bg-white hover:border-indigo-300 hover:text-indigo-600 transition-all shadow-sm">
